    ...
    def __init__(
        ...
        for col in self.dim_cols:
            if col not in self.df.columns:
                raise KeyError("dimension={} but column '{}' not found in DataFrame.".format(
                    self.dimension, col
                ))
...
def evaluate(
    ...
    log_line = "Epoch: {}\tValid Loss: {:.4f}\tValid F1 (micro): {:.4f}".format(
...
def main(seq_length: int, dimension: int, train_path: str, valid_path: str) -> None:
    ...
    for epoch in range(N_EPOCHS):
        ...
        print(
            "Epoch {}: train_loss={:.4f}, valid_loss={:.4f}, F1={:.4f}".format(
                epoch, train_loss, valid_loss, valid_f1
            )
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train wavelet-based EfficientNet model on trajectory data."
    )
