...
def evaluate(model, loader, criterion, optimizer, epoch, model_dir):
    ...
    log_line = "Epoch: {}\tLR: {:.6f}\tValid Loss: {:.4f}\tValid F1: {:.4f}".format(
...
def main(seq_length, dimension, train_path, valid_path):
    ...
    model_dir = os.path.join(MODEL_ROOT, f"len{seq_length}_dim{dimension}_cls")
    ...
    print(device)
    model = AnDiModel(input_dim=dimension, hidden_dim=64, layer_dim=3, output_dim=NUM_CLASSES).to(device)
    ...
    for epoch in range(init_epoch, n_epochs):
        ...
        valid_loss, valid_f1 = evaluate(model, valid_loader, criterion, optimizer, epoch, model_dir)
