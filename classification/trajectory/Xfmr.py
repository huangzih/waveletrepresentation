...
def evaluate(model, loader, criterion, device, epoch, history=None, model_path=None):
    ...
    status = f"Epoch: {epoch}\tValid Loss: {valid_loss:.4f}\tValid F1: {valid_f1:.4f}"
    print(status)
    if model_path is not None:
        with open(os.path.join(model_path, "log.txt"), "a+", encoding="utf-8") as f:
            f.write(status + "\n")
    return valid_loss.item(), valid_f1
...
def train_transformer(seq_length, dimension, train_path, valid_path):
    ...
    model = TransformerTimeSeriesClassifier(input_dim=dimension, num_classes=NUM_CLASSES).to(device)
    ...
    for epoch in range(init_epoch, n_epochs):
        ...
        valid_loss, valid_f1 = evaluate(model, valid_loader, criterion, device, epoch, history_valid, model_dir)
