...
def evaluate(
    ...
    log_line = f"Epoch: {epoch}\tValid Loss (MSE): {valid_loss:.4f}\tValid MAE: {mae:.4f}"
    print(log_line)
...
def main(seq_length: int, dimension: int, train_path: str, valid_path: str) -> None:
    ...
    model_dir = os.path.join(MODEL_ROOT, f"len{seq_length}_dim{dimension}_vit_reg")
    ...
    for epoch in range(N_EPOCHS):
        ...
        print(
            f"Epoch {epoch}: train_loss={train_loss:.4f}, valid_loss(MSE)={valid_loss:.4f}, MAE={mae:.4f}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train wavelet-based ViT model on trajectory data (regression)."
    )
