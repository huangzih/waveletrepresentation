...
class WaveletExtractor(nn.Module):
    def __init__(self, in_channels: int, model_name: str = BACKBONE_NAME, pretrained: bool = True):
        ...
        if not hasattr(model, "stem") or not hasattr(model.stem, "conv"):
            raise RuntimeError(f"Backbone {model_name} has no attribute 'stem.conv'.")

        old_conv = model.stem.conv
        new_conv = nn.Conv2d(
...
class WaveletModel(nn.Module):
    def __init__(
        ...
        self.extractor = WaveletExtractor(
            in_channels, model_name=backbone_name, pretrained=pretrained
        )
...
def evaluate(
    ...
    log_line = (
        f"Epoch: {epoch}\tValid Loss (MSE): {valid_loss:.4f}\tValid MAE: {mae:.4f}"
    )
    print(log_line)
...
def main(seq_length: int, dimension: int, train_path: str, valid_path: str) -> None:
    ...
    for epoch in range(N_EPOCHS):
        ...
        print(
            ...
            f"valid_loss(MSE)={valid_loss:.4f}, MAE={mae:.4f}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train wavelet-based RegNet model on trajectory data (regression)."
    )
