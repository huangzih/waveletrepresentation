...
def train_lgbm(seq_length, dimension, train_path, valid_path):
    df_train = pd.read_csv(train_path, sep=CSV_SEP)
    df_valid = pd.read_csv(valid_path, sep=CSV_SEP)

    print("There are {} samples in the training set.".format(len(df_train)))
    print("There are {} samples in the validation set.".format(len(df_valid)))
    ...
    print("Validation MAE: {:.4f}".format(mae_valid))
    print("Validation MSE: {:.4f}".format(mse_valid))
...
def main():
    parser = argparse.ArgumentParser(
        description="Train LightGBM regressor on anomalous diffusion handcrafted features."
    )
    ...
    train_lgbm(args.length, args.dimension, args.train_path, args.valid_path)
